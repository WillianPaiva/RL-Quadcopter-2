...
class Actor:
    def __init__(self, action_range, action_low, state_size=18, action_size=4,
        ...
        with tf.variable_scope(name):
            ...
            # self.x1 = tf.layers.batch_normalization(self.x1, training=True)
            # self.x1 = tf.layers.dropout(self.x1, self.dropout)
            ...
            # self.x2 = tf.layers.batch_normalization(self.x2, training=True)
            # self.x2 = tf.layers.dropout(self.x2, self.dropout)
            self.x3 = tf.layers.dense(self.x2, 32, activation=tf.nn.relu)
            # self.x3 = tf.layers.batch_normalization(self.x3, training=True)
...
class Critic:
    def __init__(self, state_size=18, action_size=1,
        ...
        with tf.variable_scope(name):
            ...
            # self.s1 = tf.layers.batch_normalization(self.s1, training=True)
            ...
            # self.a1 = tf.layers.batch_normalization(self.a1, training=True)
